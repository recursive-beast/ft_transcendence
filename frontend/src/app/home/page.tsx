      ...
      <div className="ml-[2px] flex w-full flex-col items-center rounded-lg border border-tx05 px-1 py-1 text-[9px] xs:w-52 xs:px-3 xs:py-2 xs:text-[11px] sm:w-72 sm:rounded-3xl sm:px-5 sm:text-sm md:mr-5 md:w-80 md:px-8 lg:w-72 lg:px-5 xl:mr-14 xl:w-80 xl:px-8 2xl:mr-36">
        <div>
          ...
          <div className="flex items-end">
            ...
            <div className="mb-1 text-tx02">
              Down: <span className="text-[#E55F61]">{props.down}</span>{" "}
            </div>
          </div>
        </div>
      </div>
...
export function PlayRate({ id }) {
  const { data: ratings } = useSWR(`/game/leaderboard`);

  if (!ratings) return null;

  const rating = ratings.find((elem) => elem.user.id === Number(id));
  ...
  return (
    <section className="flex justify-between xs:my-3 sm:my-4">
      <Rate
        wins={`+${rating.user.wins.length}`}
        nbrWin={rating.user.wins.length}
        wn={`${rating.winRatio.toFixed(2)}%`}
        up={`${rating.winRatio.toFixed(2)}%`}
        //
        losses={`+${rating.user.losses.length}`}
        nbrLos={rating.user.losses.length}
        los={`${rating.lossRatio.toFixed(2)}%`}
        down={`${rating.lossRatio.toFixed(2)}%`}
      />
    </section>
  );
}

function Achiv({ locked, className, ...props }: any) {
  return (
    <div
    ...
    >
      ...
      <div className="mt-4 flex items-center text-xs text-tx02 xs:mt-5 sm:text-sm">
        <Icon
          className={clsx(
            "mr-1 h-4 w-4  sm:h-5 sm:w-5",
            locked ? "text-tx06" : "text-[#24E5A5]",
          )}
          icon={locked ? "solar:lock-broken" : "solar:lock-unlocked-broken"}
        />
...
export function Achievement({ id }) {
  ...
  return (
    <section
      ref={ref}
      className="custom-scrl-bar custom-scrl-bar-achiv mb-4 grid grid-flow-col gap-2 space-x-3 overflow-x-auto overflow-y-hidden text-tx05 sm:gap-6 lg:pt-5"
    >
      <Achiv
        pic={AvMaster}
        title="Pong Novice"
        description="Play your first match"
        locked={user?.PongNovice === false}
      />
      <Achiv
        pic={AvShar}
        title="Perfect Defense"
        description="Win a game without letting the opponent score"
        locked={user?.PerfectDefense === false}
      />
      <Achiv
        pic={AvShoot}
        title="Rookie Paddler"
        description="Score your first point in a game"
        locked={user?.RookiePaddler === false}
      />
      <Achiv
        pic={AvCmBack}
        title="First Victory"
        description="First of many! Win your very first ping pong match"
        locked={user?.FirstVictory === false}
      />
      <Achiv
        pic={AvDemon}
        title="Speed Racer"
        description="Need for speed! Make the ball reach a high speed"
        locked={user?.SpeedRacer === false}
      />
      <Achiv
        pic={AvProdigy}
        title="PongMaster"
        description="Win 10 Matches"
        locked={user?.PongMaster === false}
      />
      <Achiv
        pic={AvIron}
        title="Friendly Match"
        description="Friendly competition! Play a match with a friend"
        locked={user?.FriendlyMatch === false}
      />
    </section>
  );
}
...
export function Ranking({ id }) {
  const { data: leaderboard } = useSWR("/game/leaderboard");
  const { data: user } = useSWR(`/users/${id}`);
  ...
  if (user) {
    let index = rankings.findIndex((ranking) => ranking.user.id === user.id);
    const endDiff = rankings.length - 1 - index;

    index = Math.max(0, index - 2 - (2 - Math.min(2, endDiff)));
    rankings = rankings.slice(index, index + 5);
  }
...
function SettingInput({ long, label, ...props }: any) {
  return (
...
function SettingSection({ onClick, ...props }) {
  ...
  const initialFirstName = me?.fullName.split(" ")[0] || "";
  const initialLastName = me?.fullName.split(" ").slice(1).join(" ") || "";
  ...
  async function onSave() {
    try {
      ...
      const isInvalid = status === StatusCodes.BAD_REQUEST;
      const message = isInvalid ? "Invalid input" : "Error";
  ...
  return (
    <section className="fixed inset-0 z-20 flex items-center justify-center bg-bg01/90">
      <div className="flex h-4/5 w-11/12 flex-col rounded-2xl border-[1.5px] border-tx05 bg-bg01 px-2 xs:h-3/4 xs:px-4  sm:w-[30rem] sm:px-8 lg:h-2/3 lg:w-[40rem]">
        {/* fix */}
        <div className="border-b border-tx03 pb-4 text-sm xs:pb-6 xs:text-base sm:pb-10 sm:text-lg">
          {/* top */}
          <div className="my-4 flex w-full items-center justify-between font-normal capitalize tracking-widest text-tx05 xs:my-7 sm:my-10 sm:tracking-[3px]">
            ...
            <div className="flex justify-center space-x-2 xs:space-x-4 lg:space-x-6">
              ...
              <button
                onClick={onSave}
                className="w-12 rounded-lg border border-tx01 transition-colors duration-[400ms] ease-linear hover:bg-tx01 hover:text-tx03 xs:w-16 sm:w-20 sm:p-0"
              >
          ...
          {active === "profile" && (
            <div className="flex flex-col justify-between text-sm tracking-widest xs:text-base sm:text-lg">
              ...
              <div>
                ...
                <SettingInput
                  label="User Name (max 20 characters)"
                  long
                  value={displayName}
                  onChange={(e) =>
                    setDisplayName(
...
export default function Home() {
  ...
  const firstTime = me?.firstTime === true;

  useEffect(() => {
    if (firstTime) setSetting(true);
  }, [firstTime]);

  useEffect(() => {
    axios.patch("/users/me", { firstTime: false });
  }, []);
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="mx-auto flex max-w-[1400px] flex-1 flex-col justify-between overflow-hidden">
          ...
          <div className="no-scrollbar flex flex-1 flex-col justify-between overflow-auto px-2 xs:px-3 sm:px-5 lg:px-8">
            <ProfileInfo />
            {Boolean(me) && <PlayRate id={me.id} />}
            {Boolean(me) && <Ranking id={me.id} />}
            <div>{Boolean(me) && <Achievement id={me.id} />}</div>
          </div>
        </div>
